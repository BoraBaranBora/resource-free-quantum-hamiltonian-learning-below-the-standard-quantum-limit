...
class DataGen(Dataset):
    ...
    def apply_random_gates(self, state, repetitions=0):
        """
        Apply layers of random 2-qubit unitaries to a quantum state.
        Works for any number of qubits, including odd sizes.

        Args:
            state (torch.Tensor): State vector or density matrix of shape (2^n,) or (2^n, 2^n).
            repetitions (int): Overrides self.perturbation_depth if set.

        Returns:
            torch.Tensor: The transformed quantum state.
        """
        repetitions = repetitions or self.perturbation_depth
        num_qubits = int(np.log2(state.shape[-1]))
        device = self.device

        def random_two_qubit_unitary():
            real = np.random.randn(4, 4)
            imag = np.random.randn(4, 4)
            z = real + 1j * imag
            q, _ = np.linalg.qr(z)
            return torch.tensor(q, dtype=torch.complex64, device=device)

        def embed_two_qubit_gate(U_2q, q1, q2):
            I = torch.eye(2, dtype=torch.complex64, device=device)
            ops = [I for _ in range(num_qubits)]
            q1, q2 = sorted([q1, q2])
            assert abs(q1 - q2) == 1, "Only adjacent qubits are supported in this version"
            ops[q1] = U_2q
            ops[q2] = None
        ...
        for layer_idx in range(repetitions):
            layer_type = np.random.choice(["even", "odd"])
            ...
            for q1, q2 in pairs:
                U_2q = random_two_qubit_unitary()
                U_full = embed_two_qubit_gate(U_2q, q1, q2)

                if state.ndim == 1:  # state vector
                    state = U_full @ state
                elif state.ndim == 2:  # density matrix
                    state = U_full @ state @ U_full.conj().transpose(-2, -1)
                else:
                    raise ValueError("State must be a vector or a square matrix")
