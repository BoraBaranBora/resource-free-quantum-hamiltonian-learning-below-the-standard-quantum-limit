...
def plot_errors_for_outer(
    ...
    """
    Generalized “Error vs. sum(inner_tuple)” plot for a single outer_value,
    with separate scatters and fits for each family, and consistent
    styling for SQL (x^(-0.5)) and Heisenberg (x^(-1)) curves.
    ...
    group_by : str
        One of "alpha", "times", or "spreading". Indicates which meta‐parameter is used as the “outer” grouping.
    ...
    # 3. Filter triplets where group_key == outer_value, and filter by include_families
    filtered = []
    for inner_tuple, triplets in errors_by_scaling.items():
        for (rel_err, family, group_key) in triplets:
            if group_key == outer_value:
    ...
    # 4. Build nested grouping: family → (inner_tuple → [errs])
    inner_groups_by_family = {}
    for (rel_err, family, inner_tuple) in filtered:
        inner_groups_by_family.setdefault(family, {})
        inner_groups_by_family[family].setdefault(inner_tuple, []).append(rel_err)
    ...
    # 6. Choose distinct colors and markers for families
    colors = plt.rcParams['axes.prop_cycle'].by_key()['color']
    markers = ['o', 's', '^', 'D', 'v', 'P', 'X']

    # 7. Define power‐law function for fitting
    def _power_law(x, a, b):
        return a * np.power(x, b)

    # 8. Loop over each family separately
    for idx, (family, groups) in enumerate(inner_groups_by_family.items()):
        fam_color = colors[idx % len(colors)]
        fam_marker = markers[idx % len(markers)]

        fit_x_all = []
        fit_y_all = []

        # 8a. Iterate over sorted inner_tuples for this family
        sorted_inner = sorted(
            groups.items(),
            key=lambda item: round(sum(item[0]), 8)
        )
        for inner_tuple, errs in sorted_inner:
            ssum = round(sum(inner_tuple), 8)

            # 8b. Exclude from fitting if requested
            if (exclude_x_scale is not None) and (ssum in exclude_x_scale):
                plt.scatter(
                    [ssum] * len(errs),
                    errs,
            ...
            # 8c. Prefilter logic based on scaling_param
            if scaling_param == 'times':
                if ssum < 2:
                    pref = errs.copy()
                else:
            ...
            # 8d. Percentile‐filter on pref (0th to 50th percentile)
            q0  = scoreatpercentile(pref, 0)
            q50 = scoreatpercentile(pref, 50)
            filt = [e for e in pref if (q0 <= e <= q50)]
            if len(filt) < 2:
            ...
            # 8e. Scatter raw errs for this family
            first_inner = sorted_inner[0][0]
            first_point_for_family = (ssum == first_inner)
            plt.scatter(
                [ssum] * len(errs),
                errs,
                color=fam_color,
                ...
                alpha=0.4,
                s=50,
                label=family if first_point_for_family else None
            )

            # 8f. Append filtered errs for fitting
            fit_x_all.extend([ssum] * len(filt))
            fit_y_all.extend(filt)

        # 9. Fit a power‐law if we have ≥2 points for this family
        if len(fit_x_all) >= 2 and len(fit_y_all) >= 2:
            fx = np.array(fit_x_all, dtype=float)
            fy = np.array(fit_y_all, dtype=float)
            order = np.argsort(fx)
            ...
            # Compute smooth fit curve
            x_fit = np.logspace(np.log10(fx.min()), np.log10(fx.max()), 200)
            y_fit = _power_law(x_fit, a_fit, b_fit)

            # 9a. Plot the family’s best‐fit line
            plt.plot(
                x_fit,
                y_fit,
                linestyle='--',
                color=fam_color,
                linewidth=2,
                label=f"{family} fit: b={b_fit:.2f}"
            )

            # 9b. Optionally overlay theory curves (SQL & Heisenberg)
            if show_theory:
                # Only label theory curves on the very first family iteration
                label_sql = "SQL ∝ x⁻⁰․⁵" if idx == 0 else None
                label_heis = "Heisenberg ∝ x⁻¹" if idx == 0 else None

                # Normalize both to pass through (x_fit[0], y_fit[0])
                y_sql = y_fit[0] * (x_fit / x_fit[0]) ** (-0.5)
                plt.plot(
                    x_fit,
                    y_sql,
    ...
    # 10. Finalize labels, title, and grid
    plt.xlabel(f"{inner_label} (log)", fontsize=14)
    plt.ylabel("Error (log)", fontsize=14)
    plt.title(f"Error vs {inner_label} ( {outer_label} = {outer_value} )", fontsize=16)
