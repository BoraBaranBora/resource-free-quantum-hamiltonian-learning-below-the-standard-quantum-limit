# Demo‐HamiltonianLearning with InitialStateSpreading
...
1. **Original (“manuscript”) data** (`replot_original_data/`)  
   Scripts under `replot_original_data/` that contains the _original_ data folders and produce all plots exactly as in the manuscrip:  
...
2. **Data‐(re-)generation pipelines** (`reproduce_original_data/`)  
   ...
   You can also plot Figures 1–3 (and the derivative comparison) from the reproduced data using the `composite_replotting.py` script, it orchestrates all plotting pipelines for Figures 1–3, from the datasets:
   - `first_parameter_sweep_data/` → Figure 1 data (α = 1.0; varying perturbation; measurements = 50; steps = 1…8)  
   - `second_parameter_sweep_data/` → Figure 2 data (perturb = 50; varying α; measurements = 25; steps = 1…8)  
   - `third_parameter_sweep_data/` → Figure 3 data (steps = 8; varying α and perturb; measurements = 25)  
...
3. **Source code** (`src/`)  
   ...
   - `extraction_and_evalution.py` – loads embeddings, reconstructs density matrices, computes recovery errors, fits β  
